...
use super::{ide::IdeHelper, AttributeValue, Template, TemplateWriteInstruction};
...
struct ChildParser<'a> {
	empty_elements: &'a HashSet<&'a str>,
	instructions: &'a mut Vec<TemplateWriteInstruction>,
	diagnostics: &'a mut Vec<Diagnostic>,
	ide_helper: &'a mut IdeHelper,
}

impl<'a> TemplateParser<'a> {
	pub(super) fn new(empty_elements: &'a HashSet<&'a str>) -> Self {
		Self {
			empty_elements,
	...
	pub fn parse(mut self, parsing_result: ParsingResult<Vec<Node>>) -> Template {
		...
		if let Some(nodes) = nodes {
			let mut child_parser = ChildParser {
				empty_elements: self.empty_elements,
				instructions: &mut self.instructions,
				diagnostics: &mut self.diagnostics,
				ide_helper: &mut self.ide_helper,
			};
			child_parser.visit_nodes(nodes);
		}
...
impl<'a> ChildParser<'a> {
	...
	fn visit_component(&mut self, element: NodeElement, path: Path) {
		...
		let children = if !element.children.is_empty() {
			...
			child_parser.visit_nodes(element.children);

			if instructions.is_empty() {
				None
			} else {
				Some(Children { instructions })
			}
		} else {
			None
		};
		...
		self.instructions.push(TemplateWriteInstruction::Component {
			path,
			props,
			children,
		});
